  ...
  return (
    <div className={`relative flex flex-1 items-start justify-center overflow-hidden p-8 ${pageBackground}`}>
      <div className="relative z-10 w-full">
        <div className="mb-4">
          <h1 className={`text-lg ${headingText}`}>Approved Transaction</h1>
          <p className={`text-xs ${mutedText}`}>Fully approved requests, read-only</p>
        </div>
        <TransactionsTable rows={transactionRows} showActions={false} emptyMessage="No approved transactions yet." />
      </div>
    </div>
  );
}
