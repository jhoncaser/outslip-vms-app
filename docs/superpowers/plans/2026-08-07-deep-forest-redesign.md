...
- Every existing accessible name, `role`, `aria-*` attribute, and test selector (`getByRole`, `getByLabelText`, `getByText`, etc.) must stay byte-identical, and no test's assertions/selectors may change. In practice this means test files pass unmodified — except where a task explicitly mandates a rename (e.g. Task 5's `UserRow` → `UserRowData`) that TypeScript requires cascading into a test file's own type-only import/annotation; that one-line, assertion-free edit is expected, not a violation.
...
First, rename the exported type to avoid a naming collision with the new row component added below: at the top of the file, change `export type UserRow = {...}` to `export type UserRowData = {...}`, update the `UsersView({ users }: { users: UserRowData[] })` prop type accordingly, and update `app/(authenticated)/register/page.tsx`'s import (`import { UsersView, type UserRow } from "./UsersView";` → `import { UsersView, type UserRowData } from "./UsersView";`) and its `const rows: UserRowData[] = ...` usage. A fourth place also needs it: `UsersView.test.tsx` imports the type by name too (`type UserRow` in its import statement and in its `sampleUsers` type annotation) — update both to `UserRowData` there as well, or the suite won't type-check. This is a type-only edit (no selector or assertion changes) and is expected, not a violation of the "test files pass unmodified" constraint.
